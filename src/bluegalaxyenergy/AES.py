...
_AesShiftRow = (0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11)
_AesInvShiftRow = (0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3)
_MC = ((2, 3, 1, 1), (1, 2, 3, 1), (1, 1, 2, 3), (3, 1, 1, 2))
_invMC = ((14, 11, 13, 9), (9, 14, 11, 13), (13, 9, 14, 11), (11, 13, 9, 14))


def MC(state):
    """
    Applies AES MixColumns on AES state
    :param state: AES state, as 16-byte list
...
def InvSBox(state):
    ...
    :returns: new AES state
    """
    return bytes([_AesInvSBox[x] for x in state])
...
def SBox(state):
    ...
    :returns: new AES state
    """
    return bytes([_AesSBox[x] for x in state])
...
def xor(s, k):
    return bytes([x ^ y for x, y in zip(s, k)])
...
def extendKey(key, r):
    ...
    keySize = len(key) // 4
    extKey = key
    for i in range(keySize, (r + 1) * 4):
        t = extKey[(i-1)*4:i*4]
        t2 = extKey[(i-keySize)*4:(i+1-keySize)*4]
        if i % keySize == 0:
            t = SBox(t[1:] + t[:1])
            t = xor(t, bytes([_RCon[i // keySize], 0, 0, 0]))
        elif keySize == 8 and (i % 4) == 0:
            t = SBox(t)
        extKey += xor(t, t2)

    return [extKey[16*i:16*(i+1)] for i in range(len(extKey) // 16)]


def revertKey(rKey, roundN):
    """
    Revert AES Key from AES round Key
    :param key: AES round key, as 16 (for AES-128) /24 (for AES-192) / 32 (for AES-256) bytes
    :param roundN: The round associated with this key. for AES>128, round associated with the first round key.
    :returns: The Associated AES Key
    """
    keySize = len(rKey) // 4
    extKey = rKey
    for i in range((roundN * 4) + keySize - 1, keySize - 1, -1):
        t = extKey[(keySize-2)*4:(keySize-1)*4]
        t2 = extKey[(keySize-1)*4:keySize*4]
        if i % keySize == 0:
            t = SBox(t[1:] + t[:1])
            t = xor(t, bytes([_RCon[i // keySize], 0, 0, 0]))
        elif keySize == 8 and (i % 4) == 0:
            t = SBox(t)
        extKey = xor(t, t2) + extKey[:(keySize-1)*4]
    return extKey[:keySize*4]
...
class AES:
    ...
    def __init__(self, key, r=None, addExtraMC=False, roundType=RoundType.RoundEncType0):

        assert len(key) in [16, 24, 32]
        if r is None:
            r = {16: 10, 24: 12, 32: 14}[len(key)]

        self.r = r
        self.addExtraMC = addExtraMC
        self.setKey(key)
        self.roundType = roundType

    def setKey(self, key):
        self.key = key
        self.extendedKey = extendKey(key, self.r)

    def encrypt_round(self, state, roundN):
        assert 0 <= roundN and roundN < self.r

        if self.roundType == RoundType.RoundEncType0:
            if roundN == self.r - 1:
                state = xor(state, self.extendedKey[roundN])
                state = SBox(state)
                state = ShiftRow(state)
                if self.addExtraMC:
                    state = MC(state)
                state = xor(state, self.extendedKey[self.r])
            else:
                state = xor(state, self.extendedKey[roundN])
                state = SBox(state)
                state = ShiftRow(state)
                state = MC(state)
        elif self.roundType == RoundType.RoundEncType1:
            if roundN == self.r - 1:
                state = SBox(state)
                state = ShiftRow(state)
                if self.addExtraMC:
                ...
                if roundN == 0:
                    state = xor(state, self.extendedKey[0])
                state = SBox(state)
                state = ShiftRow(state)
                state = MC(state)
                state = xor(state, self.extendedKey[roundN+1])
        elif self.roundType == RoundType.RoundEncType2:
            if roundN == self.r - 1:
                state = ShiftRow(state)
                if self.addExtraMC:
                    state = MC(state)
                ...
                state = xor(state, self.extendedKey[roundN+1])
                state = SBox(state)
        elif self.roundType == RoundType.RoundDecType0:
            if roundN == self.r - 1:
                state = MC(state)
                state = xor(state, self.extendedKey[roundN])
                state = SBox(state)
    ...
    def decrypt_round(self, state, roundN):
        assert 0 <= roundN and roundN < self.r
...
def test():
    ...
    for keyLength in [16, 24, 32]:
        for _ in range(16):
            key = random.randbytes(keyLength)
            keySched = extendKey(key, 10)
            rKey = (keySched[9] + keySched[10])[:keyLength]
